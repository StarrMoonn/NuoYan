%% fd_p2D_fs_re_wav -二维声波方程反传函数（带自由表面），保存完整波场
%
% 输入参数:
%   rec_p  - 检波器记录的地震数据
%   nx, nz - x和z方向的网格点数
...
%   - 用于可视化或其他分析目的
%
function [bp] = fd_p2D_fs_re_wav(rec_p, nx, nz, pml, dz, dx, dt, Nt, v_mod, ishot, sx, rx, water)
    % 设置计算网格参数
    Nz = nz;
    Nx = nx;
    N = Nt;
    ...
    for ii = N:-1:1
        ...
        % 波场更新（带PML边界条件）
        cur(z,x) = (2*prev(z,x) - cur(z,x).*(1-0.5*dt*(axt+azt)) + ...
                    dt^2*vel(z,x).^2.*lap)./(1+0.5*dt*(axt+azt));
        
        % 自由表面边界处理
        cur(or+1:or+SP, or+SP+1:NxT-or-SP) = ...
            (-1)*flipud(cur(spd+2:spd+SP+1, or+SP+1:NxT-or-SP));
        cur(1, or+SP+1:NxT-or-SP) = 0;
        
        % 保存当前时刻波场
        bp(:,:,ii) = extender21(cur, or+SP, or+SP, 1);

        % 每200个时间步显示一次波场
        % if(mod(ii,200)==0)  % 每200步绘制一次
        %     imagesc(cur);           % 显示当前波场
        %     colormap('gray');       % 使用灰度色图
        %     colorbar;               % 添加颜色条
        %     pause(0.1);            % 暂停0.1秒，用于观察
        % end
