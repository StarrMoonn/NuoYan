...
class SeismicCPML2DAniso:
    ...
    def setup_pml_boundary(self):
        ...
        print(f'PML各向异性稳定性判据2 (Becache et al. 2003) = {aniso2}')
    ...
    def add_source(self, it):
        ...
        # 获取震源位置的网格索引
        i = self.ISOURCE  # x方向位置
        j = self.JSOURCE  # y方向位置
        
        # 将力加入到速度场中
        # 力除以密度得到加速度，再乘以时间步长得到速度增量
    ...
    def record_seismograms(self, it):
        ...
        实现说明:
        1. 将GPU上的速度场数据转移到CPU
        2. 在每个检波器位置记录x和y方向的速度分量
        3. 存储到地震记录数组中
        """
        # 将速度场数据从GPU转移到CPU
        # 使用cupy的asnumpy函数进行数据转换
    ...
    def output_info(self, it):
        ...
        # 生成x方向和y方向速度分量的波场快照图像
        self.create_color_image(vx_cpu, it, 1)  # 生成x方向速度场图像
        self.create_color_image(vy_cpu, it, 2)  # 生成y方向速度场图像
    ...
    def save_shot_records(self):
        ...
        params = {
            ...
            'dt': self.DELTAT,
            'dx': self.DELTAX,
            'source_positions': [(self.ISOURCE, self.JSOURCE)],
            'receiver_positions': list(zip(self.rec_x, self.rec_z))
        }
