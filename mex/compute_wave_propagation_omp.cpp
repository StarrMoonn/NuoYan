...
void compute_wave_propagation_omp(
    double *vx, double *vy, 
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    // 检查输入参数数量
    if (nrhs != 35) {
        mexErrMsgIdAndTxt("MyToolbox:compute_wave_propagation_omp:nrhs",
                         "Need 35 input arguments.");
    }
    
    // 检查输出参数数量
    if (nlhs != 2) {
        mexErrMsgIdAndTxt("MyToolbox:compute_wave_propagation_omp:nlhs",
    ...
    // 调用计算函数
    compute_wave_propagation_omp(vx, vy, sigmaxx, sigmayy, sigmaxy,
