...
// 辅助函数：计算最大绝对值（需要放在主函数前面）
double computeMaxAbs(const double* array, size_t size) {
    double max_val = 0.0;
    for (size_t i = 0; i < size; i++) {
        max_val = std::max(max_val, std::abs(array[i]));
    }
    return max_val;
}

// 辅助函数：计算空间梯度
void computeGradient(const double* field, double* dx, double* dy, 
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    for (size_t it = 0; it < nt; it++) {
        // 获取当前时间步的波场数据
        const double* fwd_vx = mxGetPr(forward_vx_array) + it*nx*ny;
        const double* fwd_vy = mxGetPr(forward_vy_array) + it*nx*ny;
