...
! 在计算完X方向参数后添加输出
write(*,*) '=== X方向CPML参数 ==='
write(*,*) 'i, d_x, d_x_half, K_x, K_x_half, alpha_x, alpha_x_half, b_x, b_x_half, a_x, a_x_half'
do i = 1,NX
    write(*,'(I4,10E12.4)') i, d_x(i), d_x_half(i), K_x(i), K_x_half(i), &
                            alpha_x(i), alpha_x_half(i), b_x(i), b_x_half(i), &
                            a_x(i), a_x_half(i)
enddo

! 在计算完Y方向参数后添加输出
write(*,*) '=== Y方向CPML参数 ==='
write(*,*) 'j, d_y, d_y_half, K_y, K_y_half, alpha_y, alpha_y_half, b_y, b_y_half, a_y, a_y_half'
do j = 1,NY
    write(*,'(I4,10E12.4)') j, d_y(j), d_y_half(j), K_y(j), K_y_half(j), &
                            alpha_y(j), alpha_y_half(j), b_y(j), b_y_half(j), &
                            a_y(j), a_y_half(j)
enddo

! 添加统计信息
write(*,*) '=== X方向CPML参数统计 ==='
write(*,*) 'Parameter   Min        Max        Mean'
write(*,'(A,3E12.4)') 'd_x      ', minval(d_x), maxval(d_x), sum(d_x)/NX
write(*,'(A,3E12.4)') 'd_x_half ', minval(d_x_half), maxval(d_x_half), sum(d_x_half)/NX
write(*,'(A,3E12.4)') 'K_x      ', minval(K_x), maxval(K_x), sum(K_x)/NX
write(*,'(A,3E12.4)') 'K_x_half ', minval(K_x_half), maxval(K_x_half), sum(K_x_half)/NX
write(*,'(A,3E12.4)') 'alpha_x  ', minval(alpha_x), maxval(alpha_x), sum(alpha_x)/NX
write(*,'(A,3E12.4)') 'b_x      ', minval(b_x), maxval(b_x), sum(b_x)/NX
write(*,'(A,3E12.4)') 'a_x      ', minval(a_x), maxval(a_x), sum(a_x)/NX

write(*,*) '=== Y方向CPML参数统计 ==='
write(*,*) 'Parameter   Min        Max        Mean'
write(*,'(A,3E12.4)') 'd_y      ', minval(d_y), maxval(d_y), sum(d_y)/NY
write(*,'(A,3E12.4)') 'd_y_half ', minval(d_y_half), maxval(d_y_half), sum(d_y_half)/NY
write(*,'(A,3E12.4)') 'K_y      ', minval(K_y), maxval(K_y), sum(K_y)/NY
write(*,'(A,3E12.4)') 'K_y_half ', minval(K_y_half), maxval(K_y_half), sum(K_y_half)/NY
write(*,'(A,3E12.4)') 'alpha_y  ', minval(alpha_y), maxval(alpha_y), sum(alpha_y)/NY
write(*,'(A,3E12.4)') 'b_y      ', minval(b_y), maxval(b_y), sum(b_y)/NY
write(*,'(A,3E12.4)') 'a_y      ', minval(a_y), maxval(a_y), sum(a_y)/NY
